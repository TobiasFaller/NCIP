...
This prover supports Aiger problems with a safety properties that are specified through either:
...
E.g.: `P = R ∨ C 0 ∨ C 1 ∨ ... ∨ C n` where `R` is the allowed Aiger reset states and `C 0` to `C n` are the Craig interpolants of each step.
...
The following SAT-checks proove the certificate is valid.
We assume a stratified reset definitions (the dependency graph between resets is acyclic):
