...
class File {
  ...
  File (Message *, bool, int, int, FILE *, const char *);

  static FILE *open_file (Message *, const char *path, const char *mode);
  static FILE *read_file (Message *, const char *path);
  static FILE *write_file (Message *, const char *path);

  static void split_str (const char *, std::vector<char *> &);
  static void delete_str_vector (std::vector<char *> &);

  static FILE *open_pipe (Message *, const char *fmt, const char *path,
                          const char *mode);
  static FILE *read_pipe (Message *, const char *fmt, const int *sig,
                          const char *path);
  static FILE *write_pipe (Message *, const char *fmt, const char *path,
                           int &child_pid);
...
public:
  ...
  //
  static bool match (Message *, const char *path, const int *sig);

  // Read from existing file. Assume given name.
  //
  static File *read (Message *, FILE *f, const char *name);

  // Open file from path name for reading (possibly through opening a pipe
  // to a decompression utility, based on the suffix).
  //
  static File *read (Message *, const char *path);

  // Same for writing as for reading above.
  //
  static File *write (Message *, FILE *, const char *name);
  static File *write (Message *, const char *path);
  ...
  void connect_message (Message *m) { message = m; }
