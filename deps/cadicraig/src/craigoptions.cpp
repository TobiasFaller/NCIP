...
void Options::initialize_from_environment (int &val, const char *name,
  ...
  char key[80], *q;
  const char *p;
  assert (strlen (name) + strlen ("CADICRAIG_") + 1 < sizeof (key));
  for (p = "CADICRAIG_", q = key; *p; p++)
    *q++ = *p;
  for (p = name; *p; p++)
    *q++ = toupper (*p);
...
Options::Options (Message *m) : message (m) {
...
#define OPTION(N, V, L, H, O, P, R, D) \
  do { \
    if ((L) > (V)) \
      message->fatal ("'" #N "' default '" #V "' " \
             "lower minimum '" #L "' in 'options.hpp'"); \
    if ((H) < (V)) \
      message->fatal ("'" #N "' default '" #V "' " \
             "larger maximum '" #H "' in 'options.hpp'"); \
    if (strcmp (prev, #N) > 0) \
      message->fatal ("'%s' ordered before '" #N "' in 'options.hpp'", prev); \
    N = (int) (V); \
    assert (&val (i) == &N); \
...
void Options::set (Option *o, int new_val) {
  ...
  if (old_val == new_val) {
    message->message ("keeping value '%d' of option '%s'", old_val, o->name);
    return;
  }
  if (new_val < o->lo) {
    message->message ("bounding '%d' to lower limit '%d' for option '%s'", new_val,
         o->lo, o->name);
    new_val = o->lo;
  }
  if (new_val > o->hi) {
    message->message ("bounding '%d' to upper limit '%d' for option '%s'", new_val,
         o->hi, o->name);
    new_val = o->hi;
  }
  val = new_val;
  message->message ("set option 'set (\"%s\", %d)' from '%d'", o->name, new_val,
       old_val);
}
...
void Options::print (bool verbose) {
  ...
  if (verbose || N != (V)) { \
    if ((L) == 0 && (H) == 1) { \
      snprintf (buffer, sizeof buffer, "--" #N "=%s", \
                (N ? "true" : "false")); \
      message->message ("  %s%-30s%s (%s default %s'%s'%s)", \
      ...
      message->message ("  %s%-30s%s (%s default %s'" #V "'%s)", \
           ((N == (V)) ? "" : tout.bright_yellow_code ()), buffer, \
           ((N == (V)) ? "" : tout.normal_code ()), \
           ((N == (V)) ? "same as" : "different from"), \
           ((N == (V)) ? tout.green_code () : tout.yellow_code ()), \
...
#undef OPTION
  if (!different)
    message->message ("all options are set to their default value");
}
...
void Options::optimize (int val) {
  ...
  if (val < 0) {
    message->message ("ignoring negative optimization mode '%d'", val);
    return;
  }

  const int max_val = 31;
  if (val > max_val) {
    message->message ("optimization argument '%d' reduced to '%d'", val, max_val);
    val = max_val;
  }
...
#define OPTION(N, V, L, H, O, P, R, D) \
  do { \
    ...
    if (new_val == (int) (V)) \
      break; \
    message->message ("optimization mode '%d' for '%s' " \
...
#undef OPTION
  if (increased)
    message->message ("optimization mode '-O%d' increased %u limits", val, increased);
}
...
#define OPTION(N, V, L, H, O, P, R, D) \
  do { \
    ...
    assert ((L) == 0); \
    assert ((H) == 1); \
    message->message ("plain mode disables '%s'", #N); \
    count++; \
    N = 0; \
  } while (0);
  OPTIONS
#undef OPTION
  message->message ("forced plain mode disabled %zd preprocessing options", count);
#ifndef LOGGING
  (void) count;
#endif
}
...
#define OPTION(N, V, L, H, O, P, R, D) \
  do { \
    ...
    if (N == (V)) \
      break; \
    message->message ("resetting option '%s' to default %s", #N, #V); \
    count++; \
    N = (int) (V); \
  } while (0);
  OPTIONS
#undef OPTION
  message->message ("reset %zd options to their default values", count);
#ifndef LOGGING
  (void) count;
#endif
}
...
#define OPTION(N, V, L, H, O, P, R, D) \
  if ((N) == (int) (V)) \
    message->message ("keeping non default option '--%s=%s'", #N, #V); \
  else if ((N) != (int) (V)) { \
    message->message ("overwriting default option by '--%s=%d'", #N, N); \
    other.N = N; \
  }
  OPTIONS
#undef OPTION
