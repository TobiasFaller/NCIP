...
/*------------------------------------------------------------------------*/
#if defined(STAND_ALONE_KITTEN) || defined(LIBRARY_KITTEN)
/*------------------------------------------------------------------------*/
...
void __attribute__((weak)) kitten_error (const char *fmt, ...) {
  va_list ap;
  va_start (ap, fmt);
#if defined(STAND_ALONE_KITTEN) || defined(LIBRARY_KITTEN)
...
/*------------------------------------------------------------------------*/
#else // #ifdef defined(STAND_ALONE_KITTEN) || defined(LIBRARY_KITTEN)
/*------------------------------------------------------------------------*/
...
#if (defined(STAND_ALONE_KITTEN) || defined(LIBRARY_KITTEN)) && defined(LOGGING)
  bool logging;
#endif
  bool antecedents;
  bool learned;
...
void kitten_set_terminator (kitten *kitten, void *data, int (*terminator) (void *)) {
  REQUIRE_INITIALIZED ();
...
int main (int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (!strcmp (arg, "-h"))
      ...
      if (++i == argc)
        kitten_error ("argument to '-a' missing");
      int lit;
      if (!parse_lit (argv[i], &lit) || !lit)
        kitten_error ("invalid '-a %s'", argv[i]);
      ...
      kitten_error ("invalid option '%s' (try '-h')", arg);
    else if (output_path)
      kitten_error ("too many arguments (try '-h')");
    else if (dimacs_path)
      output_path = arg;
    else
      dimacs_path = arg;
  ...
  if (proof && !output_path)
    kitten_error ("option '-p' without output file");
  if (shrink && !EMPTY_STACK (assumptions))
    kitten_error ("can not using shrinking with assumptions");
  FILE *dimacs_file;
  ...
  if (!dimacs_path)
    ...
    kitten_error ("can not open '%s' for reading", dimacs_path);
  ...
  if (res == 10) {
    ...
    if (output_path) {
      ...
      if (!output_file)
        kitten_error ("can not open '%s' for writing", output_path);
      if (proof) {
...
static inline int less_absolute (const void *a, const void *b) {
  return ((int) (*(unsigned*) a) - (int) (*(unsigned*) b));
}

static inline unsigned to_unsigned (int lit) {
  if (lit < 0) {
    return (-lit << 1) | 1;
  } else {
    return (lit << 1);
...
void ipasir_release (void *ptr) {
  kitten_release((kitten*) ptr);
}

void ipasir_add (void *ptr, int lit) {
  unsigneds* clause = &((kitten*) ptr)->clause;

  if (lit != 0) {
    PUSH_STACK(*clause, to_unsigned(lit));
    return;
  }

  qsort(clause->begin, SIZE_STACK(*clause), sizeof(unsigned), less_absolute);
  for (size_t i = 0; i + 1 < SIZE_STACK(*clause); i++) {
    // Check for duplicate elements => remove one of them
    if (PEEK_STACK(*clause, i) == PEEK_STACK(*clause, i + 1)) {
      REMOVE_STACK(unsigned, *clause, i + 1);
      i--;
      continue;
    }

    // Check for tautologies => remove whole *clause
    if ((PEEK_STACK(*clause, i) ^ PEEK_STACK(*clause, i + 1)) == 1) {
      return;
    }
  }

  kitten_clause((kitten*) ptr, SIZE_STACK(*clause), clause->begin);
  CLEAR_STACK(*clause);
}

void ipasir_assume (void *ptr, int lit) {
  kitten_assume((kitten*) ptr, to_unsigned(lit));
}

int ipasir_solve (void *ptr) {
  return kitten_solve((kitten*) ptr);
}

int ipasir_val (void *ptr, int lit) {
  signed char value = kitten_value((kitten*) ptr, to_unsigned(lit));
  if (value < 0) {
    return -lit;
  } else if (value > 0) {
    return lit;
...
int ipasir_failed (void *ptr, int lit) {
  if (kitten_failed((kitten*) ptr, to_unsigned(lit))) {
    return 1;
  } else {
    return 0;
  }
...
void ipasir_set_terminate (void *ptr, void * data, int (*terminate)(void * data)) {
  kitten_set_terminator((kitten*) ptr, data, terminate);
}

void ipasir_trace_proof (void *ptr, FILE *file) {
